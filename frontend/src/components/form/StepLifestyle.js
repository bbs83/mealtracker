  ...
  return (
    <div className="space-y-6">
      <div className="mb-5">
        ...
        <p className="text-sm text-muted-foreground">Sua rotina diária, atividade física e hábitos</p>
      </div>
      ...
      {sleepDuration && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-primary/5 border border-primary/20" data-testid="form-sleep-duration-card">
          ...
          <div>
            <div className="text-sm font-medium">Duração estimada do sono</div>
      ...
      <div>
        <label className="text-sm font-medium mb-3 block">Nível de atividade física <span className="text-destructive">*</span></label>
      ...
      {data.activity_level && data.activity_level !== 'sedentary' && (
        <>
          <div>
            <label className="text-sm font-medium mb-1.5 block">Quais exercícios você pratica?</label>
            <p className="text-xs text-muted-foreground mb-1.5">Tipo, frequência e duração de cada um.</p>
            <Textarea placeholder="Ex: Musculação 3x/semana, corrida 2x/semana 30min" value={data.exercise_detail || ''} onChange={e => update('exercise_detail', e.target.value)} rows={3} data-testid="form-exercise-detail-input" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-medium mb-1.5 block">Horário do treino</label>
            ...
            <div>
              <label className="text-sm font-medium mb-1.5 block">Treina em jejum ou após refeição?</label>
      ...
      <div>
        <label className="text-sm font-medium mb-3 block">Nível de estresse no dia a dia <span className="text-destructive">*</span></label>
        <RadioCards options={STRESS_LEVELS} value={data.stress_level} onChange={v => update('stress_level', v)} testIdPrefix="form-stress" />
      </div>

      <div>
        <label className="text-sm font-medium mb-1.5 block">Profissão / rotina de trabalho</label>
        <p className="text-xs text-muted-foreground mb-1.5">Trabalha sentado? Em pé? Viaja muito?</p>
        <Textarea placeholder="Ex: Trabalho em escritório, sentada, home office 3x" value={data.occupation || ''} onChange={e => update('occupation', e.target.value)} rows={2} data-testid="form-occupation-input" />
      </div>

      <div className="space-y-4">
        <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
          <div className="text-sm font-medium">Consome bebida alcoólica?</div>
          <Switch checked={data.alcohol || false} onCheckedChange={v => update('alcohol', v)} data-testid="form-alcohol-switch" />
        </div>
        {data.alcohol && (
          <div className="pl-4">
            <label className="text-sm font-medium mb-1.5 block">Com que frequência?</label>
            <Input placeholder="Ex: Fins de semana, 2-3 taças de vinho" value={data.alcohol_frequency || ''} onChange={e => update('alcohol_frequency', e.target.value)} data-testid="form-alcohol-frequency-input" />
          </div>
        )}
        <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
