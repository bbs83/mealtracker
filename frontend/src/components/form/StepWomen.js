...
export default function StepWomen({ data, update }) {
  if (data.sex !== 'female') {
    return (
      <div className="text-center py-12">
        <p className="text-muted-foreground">Esta secao e apenas para pacientes do sexo feminino.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="mb-5">
        <h2 className="text-xl font-semibold mb-1" style={{ fontFamily: "'Fraunces', serif" }}>Saude Feminina</h2>
        <p className="text-sm text-muted-foreground">Informacoes hormonais e reprodutivas para personalizar melhor seu plano</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
          <div className="text-sm font-medium">Esta gravida?</div>
          <Switch checked={data.pregnant || false} onCheckedChange={v => update('pregnant', v)} data-testid="form-pregnant-switch" />
        </div>

        {data.pregnant && (
          <div className="pl-4">
            <label className="text-sm font-medium mb-1.5 block">Semanas de gestacao</label>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl border border-border bg-card">
          <div className="text-sm font-medium">Esta amamentando?</div>
        ...
        {data.contraceptive && (
          <div className="pl-4">
            ...
            <Input placeholder="Ex: Pilula, DIU, implante..." value={data.contraceptive_type || ''} onChange={e => update('contraceptive_type', e.target.value)} data-testid="form-contraceptive-type-input" />
          </div>
        )}
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-1.5">TPM intensa, retencao de liquido, compulsao alimentar em alguma fase do ciclo...</p>
        <Textarea placeholder="Descreva sintomas hormonais que voce percebe..." value={data.hormonal_symptoms || ''} onChange={e => update('hormonal_symptoms', e.target.value)} rows={3} data-testid="form-hormonal-symptoms-input" />
      </div>
    </div>
  );
}
